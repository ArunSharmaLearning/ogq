...
const Banner = ({ image, text, subText, sx = {} }: { image: string, text: string, subText?: string, sx?: object }) => {
	...
	return (
		<Box sx={{ p: 1, display: 'flex', position: 'relative', height: '22rem', backgroundImage: `url(${image.startsWith('http') ? image : '/' + image})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundPosition: 'center bottom', textAlign: 'center', backgroundAttachment: 'fixed', mb: { xs: 2, sm: 3 }, ...sx }}>
